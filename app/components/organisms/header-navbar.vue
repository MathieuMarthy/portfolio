...
const navItems = [
    ...
    { key: "my-experiences", href: "#my-experiences" },
...
<template>
    ...
    <div class="flex w-full justify-between p-8 fixed max-md:hidden">
        <atoms-theme-toggle :is-dark="isDark" class="rounded-2xl" @toggle="toggleTheme" />

        <nav ref="navRef" class="flex items-center justify-end gap-8 bg-background-alt rounded-2xl px-5 py-2 relative">
            <a v-for="item in navItems" :key="item.key" :class="[
    ...
    <div class="md:hidden fixed top-0 left-0 right-0 z-50">
        ...
        <div class="flex w-full justify-between items-center p-4 bg-background/80 backdrop-blur-md">
            <atoms-theme-toggle :is-dark="isDark" class="rounded-xl" svg-class="w-5 h-5" @toggle="toggleTheme" />

            <!-- Hamburger Button -->
            <button aria-label="Toggle menu" class="flex flex-col justify-center items-center gap-1.5
                    bg-background-alt rounded-xl p-2.5 cursor-pointer" @click="toggleMobileMenu">
                <span :class="[
        ...
        <Transition enter-active-class="transition-opacity duration-300 ease-out" enter-from-class="opacity-0"
            ...
            <div v-if="isMobileMenuOpen" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-40"
        ...
        <Transition enter-active-class="transition-all duration-300 ease-out"
            ...
            leave-active-class="transition-all duration-300 ease-in" leave-from-class="translate-x-0 opacity-100"
            ...
            <nav v-if="isMobileMenuOpen"
                ...
                <div class="flex justify-end p-4">
                    <button aria-label="Close menu" class="p-2 rounded-xl bg-background text-text cursor-pointer"
                        @click="closeMobileMenu">
                ...
                <div class="flex flex-col gap-2 px-6 py-4">
                    <a v-for="item in navItems" :key="item.key" :class="[
                        ...
                        activeSection === item.href
                            ...
                            : 'hover:bg-background'
                    ]" :href="item.href" @click="closeMobileMenu">
                        {{ $t(`navbar.${item.key}`) }}
                    </a>
                </div>
            </nav>
